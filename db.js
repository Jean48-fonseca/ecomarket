...
(function(global){
  ...
  const ADMIN_DB_VERSION = 2; // Incrementar versión para migrar reclamos
  ...
  async function dbOpen(){
    ...
    req.onupgradeneeded = (e)=>{
      ...
      // Reclamos: key autoincrement, ticket único
      if(!db.objectStoreNames.contains('reclamos')){
        const s3 = db.createObjectStore('reclamos', { keyPath: 'id', autoIncrement: true });
        s3.createIndex('ticket', 'ticket', { unique: true });
        s3.createIndex('dni', 'dni', { unique: false });
        s3.createIndex('estado', 'estado', { unique: false });
      }
    };
  ...
  // Reclamos
  async function guardarReclamo(reclamo){
    const db = await dbOpen();
    return new Promise((res, rej)=>{
      const tx = db.transaction('reclamos','readwrite');
      const store = tx.objectStore('reclamos');
      const now = new Date().toISOString();
      const payload = {...reclamo, creado_en: now, estado: 'pendiente', respuesta: ''};
  ...
  async function listarReclamos(filtro={}){
    const db = await dbOpen();
    return new Promise((res, rej)=>{
      ...
      req.onsuccess = ()=>{
        let out = req.result||[];
        if(filtro.ticket) out = out.filter(r=>r.ticket===filtro.ticket);
        if(filtro.dni) out = out.filter(r=>r.dni===filtro.dni);
        if(filtro.estado) out = out.filter(r=>r.estado===filtro.estado);
        res(out);
      };
      req.onerror = ()=>rej(req.error);
    });
  ...
  async function responderReclamo(id, respuesta){
    const db = await dbOpen();
    return new Promise((res, rej)=>{
      const tx = db.transaction('reclamos','readwrite');
      const store = tx.objectStore('reclamos');
      ...
      req.onsuccess = ()=>{
        ...
        if(!rec){ rej('No existe el reclamo'); return; }
        rec.respuesta = respuesta;
        rec.estado = 'respondido';
        rec.respondido_en = new Date().toISOString();
  ...
  global.EcoDB = {
    ...
    guardarReclamo, listarReclamos, responderReclamo
  };
})(window);
